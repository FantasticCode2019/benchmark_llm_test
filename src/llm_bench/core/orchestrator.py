...
def _step_probe_ollama_thinking(ctx: BenchmarkContext) -> None:
    ...
    if ctx.opts.api_type is not ApiType.OLLAMA:
        return
    try:
        # The probe internally resolves the daemon-authoritative model
        # id via /v1/models, so configured names with annotation
        # suffixes like "(Unsloth GGUF)" no longer 4xx /api/show — see
        # `ollama_supports_thinking` docstring for the discovery flow.
