...
def _merge_install_envs(envs: list | None) -> list[str]:
    """Normalise caller-supplied ``--env`` entries for ``market install``.

    No defaults are injected here — the harness no longer ships any
    built-in env (e.g. the previous ``OLARES_USER_HUGGINGFACE_SERVICE``
    default has been removed). Whatever the caller passes through
    ``ensure_installed(install_envs=...)`` is the full set forwarded
    to ``olares-cli market install --env``.

    Each entry is a ``"KEY=VALUE"`` string passed straight through to
    the CLI; the CLI handles its own quoting so we deliberately do
    NOT shell-escape here. Behaviour:

      * KEY=VALUE entries are deduplicated by key (last write wins,
        matching the prior caller-override semantics).
      * Strings without an ``=`` pass through verbatim and are NOT
        deduplicated — preserves the rare path where a caller wants
        to forward an arbitrary ``--env`` flag that doesn't fit the
        KEY=VALUE shape.
      * Non-string entries are silently dropped (defensive: keeps a
        misconfigured row from crashing the install).

    The returned list places keyed ``KEY=VALUE`` entries first
