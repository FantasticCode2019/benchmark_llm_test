...
一个**端到端、可被 cron 触发的 LLM 基准测试脚本**，用来批量评估 Olares 上以 Ollama / vLLM / LLaMA.cpp chart 形式部署的大模型。运行环境基于 **Python 3.11+**，绝大部分功能直接用标准库（`urllib` / `subprocess` / `smtplib` / `json` 等）；vLLM 的 thinking 探针走官方 `openai` SDK。

```bash
python3 -m venv venv && source venv/bin/activate
pip install -e .                # 安装可执行入口 + 运行时依赖
pip install -e ".[dev]"         # 可选：装 pytest / ruff / mypy
```
...
采用标准 **src/ layout**（[PEP 518 / pyproject.toml](https://peps.python.org/pep-0518/)）：

```text
benchmark_llm_test/
├── pyproject.toml                   # 包元数据 + ruff / pytest / mypy 配置
├── requirements.txt                 # 运行时 deps（仅 openai>=1.30）
├── llm_bench.py                     # 旧 cron 兼容 shim（python3 llm_bench.py ...）
...
│       ├── constants.py             # 集中常量（GIB_BYTES / 轮询间隔 / 超时 / 日志命名空间 / …）
│       ├── exceptions.py            # BenchmarkError 异常层级（root: BenchmarkError(RuntimeError)）
│       ├── models.py                # 兼容 shim：re-export 自 llm_bench.domain
│       ├── domain/                  # 数据层（纯 dataclass + StrEnum）
│       │   ├── enums.py             #   ApiType / InstallDecision（StrEnum 保持 JSON 字符串）
│       │   ├── results.py           #   QuestionResult / ModelResult（基准测试输出）
│       │   └── config.py            #   AppConfig / GlobalDefaults / ModelSpec / EmailConfig
│       │                            #   / ResolvedOptions / OpenAIConfig（基准测试输入）
│       ├── core/                    # 核心业务逻辑
│       │   ├── _context.py          #   BenchmarkContext（orchestrator 私有状态对象）
│       │   ├── lifecycle.py         #   chart 装/卸 + ensure_installed + 状态桶 + 日志归档
...
│       │   ├── report_writer.py     #   write_reports (JSON + HTML 落盘)
│       │   ├── mailer.py            #   SMTP (implicit TLS / STARTTLS + 重试)
│       │   └── probe.py             #   --probe: chart env 需求 dump
│       ├── clients/                 # 协议客户端（按后端区分）
│       │   ├── ollama_client.py     #   /api/ps + /api/tags + /api/show + /api/progress + describe/supports_thinking
│       │   ├── vllm_client.py       #   /cfg + /progress?id=… (SSE 兼容)
│       │   └── openai_errors.py     #   OpenAIHTTPError + post_openai + auth_hint
│       └── utils/                   # 通用工具
...
└── tests/                           # smoke 单测（pytest，纯函数 + dataclass 边界）
    ...
    ├── test_tokens.py
    ├── test_time_utils.py
    ├── test_ollama_helpers.py
    ├── test_enums.py                # ApiType / InstallDecision 序列化
    ├── test_config_loading.py       # AppConfig.from_dict / ResolvedOptions 解析
    └── test_orchestrator_helpers.py # BenchmarkContext + 日志格式
```
...
CLI 参数完全没变（`-c` / `--log` / `--cli-path` / `--no-email` / `--probe`），三种等价的调用方式选一种即可：

| 调用方式 | 适用场景 |
|---|---|
| `llm-bench …`               | `pip install -e .` 之后，最干净 |
| `python3 -m llm_bench …`    | 同上效果，但**必须在项目根目录以外**执行（项目根目录有 `llm_bench.py` 兼容 shim，会跟包同名冲突） |
| `python3 llm_bench.py …`    | 老 cron 任务不用改，shim 会把 `src/` 加到 `sys.path` 后转发到 `llm_bench.cli:main` |

```bash
# 跑完整流程
llm-bench -c config.example.json

# 跑测试，不发邮件
llm-bench -c config.example.json --no-email

# 仅 dump 每个 chart 的 env 需求（不安装、不卸载）
llm-bench -c config.example.json --probe
...
推荐做法：`pip install -e /path/to/benchmark_llm_test` 之后直接用 `llm_bench` 包。`load_config` 返回**强类型** `AppConfig` dataclass（不再是 dict），`bench_model` 接受 `ModelSpec` + `list[str]` + `AppConfig`：

```python
from llm_bench.data.config import load_config
from llm_bench.core.orchestrator import bench_model
from llm_bench.domain import ModelResult       # 新规范路径
# from llm_bench.models import ModelResult     # 旧路径仍可用（re-export shim）

cfg = load_config("config.example.json")        # -> AppConfig
results: list[ModelResult] = [
    bench_model(spec, cfg.questions, cfg)
    for spec in cfg.models
]
```

字段以 dataclass 属性方式访问：`cfg.questions` / `cfg.models[i].app_name` / `cfg.defaults.install_timeout_minutes` / `cfg.email.smtp_host` 等。如果配置文件本身有问题（缺字段、JSON 不合法），会抛 `llm_bench.exceptions.ConfigError`（含 `ConfigValidationError` 子类），CLI 入口已经把它转换成友好退出码。

如果不能/不想 `pip install`，也可以把 `src/` 直接挂到 `sys.path`：

```python
import sys, pathlib
sys.path.insert(0, str(pathlib.Path("/path/to/benchmark_llm_test") / "src"))
...
> ⚠️ 注意：项目根的 `llm_bench.py` 是 cron 兼容 shim，**和 `src/llm_bench/` 包同名**。导入时不要把项目根放到 `sys.path` 第 0 位（会把 shim 当成模块），只把 `src/` 加进去就行。
...
# Lint（已配置 ruff，遵循 PEP 8 + pyupgrade + isort + bugbear）
ruff check src tests llm_bench.py
ruff format src tests llm_bench.py    # 可选：格式化
```

测试覆盖：

- **纯函数**：`utils.format` / `utils.tokens` / `utils.time_utils` / `clients.ollama_client` 的 jq 投影 helpers
- **数据层**：`ApiType` / `InstallDecision` 枚举的 JSON 序列化往返（保证报告文件向后兼容）
- **配置层**：`AppConfig.from_dict` 必填字段校验、`ResolvedOptions.for_model` 覆盖语义（含 `False` / `0` 不被默认值误覆盖的回归测试）、`load_config` 文件错误处理
- **编排层**：`BenchmarkContext` 状态聚合 + `_log_prompt_result` 两条 backend 日志格式

共 **72 条 unit test**（全部 < 1s 跑完）；网络 / subprocess / SMTP 等带副作用的入口暂未 mock 化（需要 `olares-cli` 实测环境），可结合 `--probe` 子命令做集成验证。
